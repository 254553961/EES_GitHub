...
void AudioPllConfig() {
	...
	Status = IicConfig(XPAR_PS7_I2C_1_DEVICE_ID);//, FMC_IIC_ID, HDMI_IIC_ID);
	...
	if(Status != XST_SUCCESS) {
		xil_printf("IIC XST_FAILURE\r\n");
		return XST_FAILURE;
	}
	xil_printf("IIC XST_SUCCESS\r\n");
...
void AudioConfigureJacks()
{
	...
	AudioWriteToReg(R29_PLAYBACK_HEADPHONE_LEFT_VOLUME_CONTROL, 0xE7);//Mute Left channel of HP port (LHP)
	AudioWriteToReg(R30_PLAYBACK_HEADPHONE_RIGHT_VOLUME_CONTROL, 0xE7); //Mute Right channel of HP port (LHP)
...
void micconfig() {
		...
		AudioWriteToReg(R4_RECORD_MIXER_LEFT_CONTROL_0, 0x5B);
		AudioWriteToReg(R5_RECORD_MIXER_LEFT_CONTROL_1, 0x08);//0 dB gain
		AudioWriteToReg(R6_RECORD_MIXER_RIGHT_CONTROL_0, 0x5B);
		AudioWriteToReg(R7_RECORD_MIXER_RIGHT_CONTROL_1, 0x08);//0 dB gain
		AudioWriteToReg(R8_LEFT_DIFFERENTIAL_INPUT_VOLUME_CONTROL, 0x83);
		AudioWriteToReg(R9_RIGHT_DIFFERENTIAL_INPUT_VOLUME_CONTROL, 0x83);//0 dB gain
		AudioWriteToReg(R10_RECORD_MICROPHONE_BIAS_CONTROL, 0x01);//0 dB gain
